...
def detect():
    ...
    try:
        ...
        if len([box for box in boxes if box[4] in ["person", "Person"]]) == 1:
            ...
            real_or_spoof_status = 'Real' if real_or_spoof[0] > 0.5 else 'Spoof'  # Adjust threshold if needed
            safety_status += f" | Real/Spoof: {real_or_spoof_status}"
        
        # Return plain text with the format "Result: ... "
        return Response(
            f"{safety_status}",  
            mimetype='text/plain'
        )
...
def detect_objects_on_image(image):
    ...
    # Determine messages based on counts
    if person_count > 1:
